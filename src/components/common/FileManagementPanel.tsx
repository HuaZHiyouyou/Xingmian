...
import { fileBlobUrl, isRunningInTauri } from '../../lib/tauriBridge';
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="px-3 py-1.5 text-xs rounded-xl border dark:border-gray-600 bg-white dark:bg-gray-800 hover:border-slate-400 dark:hover:border-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-700 transition-all flex items-center gap-1 min-w-[80px]"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute z-20 mt-1 left-0 w-full bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-xl shadow-lg overflow-hidden py-1 animate-[fadeIn_0.15s_ease-out]">
            {options.map(opt => (
              <button
                key={opt.value}
                ...
                className={`w-full px-3 py-2 text-xs text-left transition-colors ${
                  value === opt.value
                    ? 'bg-slate-100 dark:bg-slate-800/40 text-slate-800 dark:text-slate-300 font-medium'
...
function FilePreview({ file, onClose }: { file: FileRecord; onClose: () => void }) {
  // 🆕 性能：走 file-blob 协议直出二进制（WebView 原生加载+缓存），不再经 base64/IPC
  const [loading, setLoading] = useState(true);
  const src = isRunningInTauri() ? fileBlobUrl(file.id) : '';

  useEffect(() => {
    if (!isRunningInTauri()) return undefined;
    setLoading(true);
    const timer = setTimeout(() => setLoading(false), 8000); // 兜底：协议异常时不无限转圈
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-[90vw] max-h-[90vh] overflow-hidden animate-[scaleIn_0.2s_ease-out]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 flex items-center justify-center min-h-[200px] max-h-[70vh] overflow-auto">
          {file.mimeType.startsWith('image/') ? (
            <>
              {loading && (
                <div className="flex flex-col items-center gap-3">
                  <div className="w-10 h-10 border-2 border-slate-700 border-t-transparent rounded-full animate-spin" />
            ...
            <div className="w-80">
              <div className="text-center mb-4"><Music size={32} className="text-green-500 mx-auto" /></div>
...
function ImageThumbnail({ file }: { file: FileRecord }) {
  ...
  // 🆕 性能：缩略图走 file-blob 协议（二进制直出+HTTP 缓存），不再逐张 base64 IPC
  const src = isRunningInTauri() ? fileBlobUrl(file.id) : '';
...
export function FileManagementPanel() {
  const { files, hasMore, isLoading, stats, loadFiles, loadMore, softDeleteFile, hardDeleteFile, setFilter, loadStats } = useFileStore();
  ...
  useEffect(() => {
    if (!isRunningInTauri()) return undefined;
    // 🆕 性能：focus 刷新节流到 30s 一次（此前每次切回窗口都重拉列表 + 全表 SUM 统计）
      ...
      <div className="flex-1 overflow-y-auto p-4" onScroll={handleScroll}>
        ...
        {isLoading && initialLoaded && (
          <div className="text-center py-4">
            <div className="inline-block w-5 h-5 border-2 border-slate-700 border-t-transparent rounded-full animate-spin" />
          </div>
        )}

        {hasMore && !isLoading && (
          <button onClick={loadMore} className="w-full py-2 text-xs text-slate-700 dark:text-slate-300 hover:text-slate-700">
            加载更多...
          </button>
        )}
      </div>
