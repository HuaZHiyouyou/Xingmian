...
export function APIConfigPage() {
  ...
  const [newModelType, setNewModelType] = useState<ModelType>('chat');
        ...
        <div className="space-y-4">
                ...
                >
                  <button
                    ...
                    className={`w-11 h-6 rounded-full transition-all duration-300 relative flex-shrink-0 ${
                      p.enabled ? 'bg-violet-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                  >
                    <div
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {!p.isDefault && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (window.confirm(`确定要删除平台 "${p.displayName}" 吗？`)) {
                            removePlatform(i);
                            ...
                            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all shadow-sm flex items-center gap-2 ${
                              selectedModelKey
                                ? (() => {
                                    ...
                                    if (!m || !m.pinned) return 'bg-violet-600 hover:bg-violet-700 text-white';
                                    return m.enabled
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center gap-3 p-3 rounded-xl transition-colors duration-150 relative ${
                                    isSelected
                                      ? 'bg-violet-50 dark:bg-violet-900/10 border border-violet-300 dark:border-violet-700 ring-1 ring-violet-200 dark:ring-violet-600'
                                      : model.enabled
                                      ? 'bg-violet-50/60 dark:bg-violet-900/10 border border-violet-150 dark:border-violet-800/30'
                                      : 'bg-white dark:bg-gray-800 border border-transparent hover:border-gray-200 dark:hover:border-gray-700'
                                  }`}
                                >
                                  {model.enabled && (
                                    <div className="absolute -left-0.5 top-1/2 -translate-y-1/2 w-1 h-6 bg-violet-500 rounded-r-full" />
                                  )}
                                  <button
                                    onClick={() => {
                                  ...
                                  >
                                    <Pin size={12} className={`flex-shrink-0 ${
                                      isPinned ? 'text-violet-500' : 'text-gray-300 dark:text-gray-600'
                                    }`} />
                                    ...
                                    {isPinned && (
                                      <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 flex-shrink-0 font-medium">
                                        已固定
                                      </span>
                                    )}
                                  </button>
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs shadow-sm transition-all cursor-pointer ${
                                        isThisSelected
                                          ? 'bg-violet-50 dark:bg-violet-900/30 border-violet-300 dark:border-violet-600 ring-1 ring-violet-200 dark:ring-violet-500'
                                          : isEnabled
                                          ? 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-violet-300 dark:hover:border-violet-500'
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl border-2 border-dashed
              ...
              hover:border-violet-400 hover:text-violet-500 dark:hover:border-violet-500 dark:hover:text-violet-400
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-medium
                      hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
                  >
                    添加
                  </button>
                </div>
