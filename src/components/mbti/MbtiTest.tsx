        ...
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-8">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm mb-4">
            <div className="text-center mb-6">
              ...
              <div className="text-3xl font-bold text-violet-600 dark:text-violet-400 mb-1">{result.type}</div>
                ...
                return (
                  <div key={dim} className="pb-4 border-b border-gray-100 dark:border-gray-800 last:border-0 last:pb-0">
                    <div className="flex justify-between text-[10px] text-gray-400 dark:text-gray-500 mb-1">
                      <span className={leftActive ? 'font-bold text-violet-500' : ''}>{dimInfo.left}</span>
                      <span className="text-[9px] opacity-60">{dimInfo.label}</span>
                      <span className={!leftActive ? 'font-bold text-violet-500' : ''}>{dimInfo.right}</span>
                    </div>
                    <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden flex mb-2">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${leftPercent}%`, backgroundColor: leftActive ? '#8b5cf6' : '#d1d5db' }} />
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${rightPercent}%`, backgroundColor: !leftActive ? '#8b5cf6' : '#d1d5db' }} />
                    </div>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">
                      <span className="font-medium text-violet-500">{lean}</span> {absVal > 60 ? '倾向明显' : absVal > 20 ? '略偏' : '较均衡'}（{absVal}%）· {analysis}
                    </p>
                  </div>
                );
              })}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/chat')} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition-colors">
              返回聊天
              <ArrowRight size={16} />
            </button>
          </div>
      ...
      <div className="flex-1 bg-gray-50 dark:bg-gray-950 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-8">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm mb-4">
              ...
              {Object.entries(DIMENSION_LABELS).map(([, v]) => (
                <div key={v.label} className="px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-violet-500 font-medium">{v.label}</span>
                  </div>
            ...
            <button onClick={() => setShowIntro(false)} className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition-colors">
              开始测试
            </button>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center shrink-0 mt-0.5">
                  <span className="text-[10px] font-bold text-violet-600 dark:text-violet-400">1</span>
                </div>
                <div>
              ...
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center shrink-0 mt-0.5">
                  <span className="text-[10px] font-bold text-violet-600 dark:text-violet-400">2</span>
                </div>
                <div>
              ...
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center shrink-0 mt-0.5">
                  <span className="text-[10px] font-bold text-violet-600 dark:text-violet-400">4</span>
                </div>
                <div>
              ...
              {(Object.entries(MBTI_DESCRIPTIONS) as [MbtiType, typeof MBTI_DESCRIPTIONS[MbtiType]][]).map(([type, info]) => (
                <div key={type} className="px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <div>
                      <span className="text-xs font-bold text-violet-600 dark:text-violet-400">{type}</span>
                  ...
                  <div className="flex flex-wrap gap-1.5 text-[10px]">
                    <span className="px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">优势：{info.strengths}</span>
                    ...
                    <div key={record.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800/50 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-bold text-violet-600 dark:text-violet-400">{record.type}</span>
  ...
  return (
    <div className="flex-1 bg-gray-50 dark:bg-gray-950 overflow-y-auto">
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-8">
        ...
        {question && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm mb-4">
            <div className="text-[10px] text-violet-500 uppercase tracking-wide mb-2 font-medium">
        ...
        <div className="flex justify-between">
          ...
          {isLastQuestion && hasAnswered && (
            <button onClick={calculateResult} className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition-colors">
              查看结果
              <ArrowRight size={16} />
            </button>
          )}
