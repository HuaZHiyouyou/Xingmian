  ...
  try {
    ...
    if (data && data.profiles && Array.isArray(data.profiles)) {
      ...
      for (const [charId, profile] of Object.entries(data)) {
        const p = profile as { vocabulary?: unknown[]; phrases?: unknown[] } | null;
        if (p && typeof p === 'object') {
          result[charId] = {
            vocabulary: (Array.isArray(p.vocabulary) ? p.vocabulary : []) as string[],
            phrases: (Array.isArray(p.phrases) ? p.phrases : []) as string[],
          };
        }
      }
    }
