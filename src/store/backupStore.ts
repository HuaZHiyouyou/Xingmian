...
export const useBackupStore = create<BackupState>((set, get) => ({
  ...
  restoreBackup: async (backupId: string) => {
    ...
    if (isRunningInTauri()) {
      ...
      try {
        ...
        const found = all.find((b: any) => b.id === backupId);
    ...
    if (data.memories) {
      ...
      if (isRunningInTauri()) {
        const a = Object.values(data.memories).flat();
        if (a.length > 0) await dbSaveMemories(a as any[]);
      }
    }
    if (data.reflections) {
      useCharacterMindStore.setState({ reflections: data.reflections });
      if (isRunningInTauri()) {
        const a = Object.values(data.reflections).flat();
        if (a.length > 0) await dbSaveReflections(a as any[]);
      }
    }
    if (data.entries) {
      useMemoryStore.setState({ entries: data.entries });
      if (isRunningInTauri()) {
        const a = Object.values(data.entries).flat();
        if (a.length > 0) await dbSaveMemoryEntries(a as any[]);
      }
    }
    ...
    if (data.userProfile) {
      ...
      if (isRunningInTauri()) {
        ...
        await dbSaveUserProfile({ avatar: p.avatar || '', nickname: p.nickname || '', age: p.age || '', gender: p.gender || '', mbti: p.mbti || '', personality: p.personality || '', background: p.background || '', interests: p.interests || '', habits: p.habits || '', notes: p.notes || '' });
      }
    }
    if (data.platforms) {
  ...
  hardDeleteBackup: async (backupId: string) => {
    if (isRunningInTauri()) {
      ...
      try {
        const all = JSON.parse(localStorage.getItem(LS_BACKUPS_KEY) || '[]');
        localStorage.setItem(LS_BACKUPS_KEY, JSON.stringify(all.filter((b: any) => b.id !== backupId)));
      } catch { /* ignore */ }
    }
    await get().loadBackups();
  },
  ...
  exportBackupToFile: async (backupId: string) => {
    ...
    if (isRunningInTauri()) {
      ...
      try {
        const all = JSON.parse(localStorage.getItem(LS_BACKUPS_KEY) || '[]');
        const f = all.find((b: any) => b.id === backupId);
        if (f) { dataJson = f.dataJson; label = f.label || backupId; }
      } catch { /* ignore */ }
    }
    if (!dataJson) return;
  ...
  exportAllBackups: async () => {
    ...
    for (const backup of backups) {
      ...
      if (isRunningInTauri()) {
        ...
        try {
          const all = JSON.parse(localStorage.getItem(LS_BACKUPS_KEY) || '[]');
          const f = all.find((b: any) => b.id === backup.id);
          if (f) dataJson = f.dataJson;
        } catch { /* ignore */ }
      }
      if (dataJson) {
