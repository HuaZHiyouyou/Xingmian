...
1. 所有回复必须用中文，禁止输出英文
2. 每次只问一个问题，问题要简洁
3. 根据用户之前的回答调整后续问题——问细节，而不是让用户自己编故事
4. 核心原则：只记录用户告诉你的内容，不添加你脑补的信息。用户没说的事不要自己编
5. 当用户说"创建角色"、"确认"、"正确"、"没问题"时，立即输出JSON格式的角色数据
...
const QUESTIONS = [
  '这个角色叫什么名字？是什么样的人？（一句话概括）',
  '角色的性格底色是什么？用一段话描述，不要只堆标签。',
  '角色的背景设定是怎样的？（不用太复杂，知道核心设定就行）',
  '角色有什么喜欢和讨厌的事物？具体一点。',
  '角色有什么小习惯或独特的表达方式？',
  '角色在不同情绪下分别是什么样子？（开心时？难过时？生气时？）',
  '角色的思考方式是怎样的？（乐观/悲观/理性/直觉？遇到事情先想什么？）',
  '角色的说话风格是怎样的？（话多话少？语气轻快还是沉稳？有没有常用语气词？）',
  '角色和不同熟悉程度的人说话方式会不一样吗？（刚认识vs熟悉vs亲密）',
  '有什么是这个角色绝对不能做或绝对不会说的？',
];
...
1. 所有输出必须是中文
2. 禁止输出英文或分析过程
3. 只从文件中提取已有信息，不要脑补、不要添加文件里没有的内容
4. 只输出以下格式，不要其他内容
...
没找到的字段写"未找到"。最后问"这些信息正确吗？确认后我来生成角色。"`;
      ...
      const reply = await callAI(aiMessages, fileAnalysisPrompt, 4096, undefined, 'background');
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const reply = await callAI(aiMessages, systemPrompt, 8192, undefined, 'background');
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[78%] ${msg.role === 'user' ? 'order-1' : ''}`}>
                {/* 文件卡片 */}
                {msg.file ? (
                  <div className="inline-flex items-center gap-3 px-4 py-3 rounded-2xl bg-gradient-to-r from-slate-700 to-slate-700 text-white shadow-sm">
